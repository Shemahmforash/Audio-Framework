...
public class WaveDecoder implements AudioDecoder {
	...
	/**
	 * The inverse of the maximum value allowed for a short
	 */
	private final float INVERSE_MAX_VALUE = 1.0f / Short.MAX_VALUE;
	...
	public WaveDecoder(InputStream stream) throws Exception {
		...
		// TODO: suporte para outro bit-rate
		...
		for (int i = 0; i < samples.length; i++) {
			...
			try {
				...
				for (int channel = 0; channel < channels; channel++) {
					...
					sample += (shortValue * INVERSE_MAX_VALUE);
				}
				sample /= channels;
				samples[i] = sample;
				numSamples++;
